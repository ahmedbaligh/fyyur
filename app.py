...
def venues():
    ...
    areas = db.session.query(func.count(Venue.id), Venue.city, Venue.state).group_by(Venue.city, Venue.state).all()
    data = []

    for area in areas:
        venuesByArea = Venue.query.filter_by(state=area.state).filter_by(city=area.city).all()
        venueData = []
...
def search_artists():
    ...
    search_term = request.form.get('search_term', '')
    results = Artist.query.filter(name.ilike(f'%{search_term}%')).all()
    data = []
...
def edit_artist_submission(artist_id):
    ...
    try:
        ...
        flash('An error occurred. Artist ' + artistData.name.data + ' could not be updated.')
    finally:
        db.session.close()
...
def edit_venue_submission(venue_id):
    ...
    try:
        ...
        print(sys.exc_info())
        # Done: on unsuccessful db update, flash an error instead.
        flash('An error occurred. Venue ' + venueData.name.data + ' could not be updated.')
    finally:
        db.session.close()
...
def create_show_submission():
    ...
    try:
        newShow = Show(start_time=showData.start_time.data,
